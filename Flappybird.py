...
class Flappybird:
    def __init__(self):
        ...
        self.flap_sound = pygame.mixer.Sound('assets\sound\Flap.ogg')
        self.die_sound = pygame.mixer.Sound('assets\sound\Hit.ogg')
        self.fall_sound = pygame.mixer.Sound('assets\sound\Fall.ogg')
    ...
    def pipe_move(self):
        ...
        if self.pipeX <= 84 and (self.pipeY-15 < self.birdY  or self.pipeY-120+15 > self.birdY):
            ...
            if self.die:
                ...
                pygame.mixer.Sound.play(self.fall_sound)
                self.die = False
    ...
    def scoreup(self):
        ...
        if self.score<10:
            ...
            self.showscore(trunc(self.score/10),-13)
            self.showscore(self.score-(trunc(self.score/10)*10),13)
        elif self.score<1000:
            self.showscore(trunc(self.score/100),-25)
            self.showscore((self.score - (self.score - (trunc(self.score/10)*10)) - (trunc(self.score/100)*100))/10,0)
            self.showscore(self.score - (trunc(self.score / 10)*10), 25)
    def show(self):
        self.flap()
        self.pipe_move()
        self.birdScript+=1
